...
bool Optimize(const Eigen::Matrix3d                           &init,
  ...
  if (is_solved) {
    _optimizer.getTrajectory(&traj_);
  } else {
    ROS_ERROR("Optimization failed!");
    return false;
  }
  ...
  /* reoptimization */
  int i = 0, I = 10;
  while (!_optimizer.isCorridorSatisfied(traj_, 10.0, 10.0, 0) && i++ < I) {
    try {
...
void clickCallback(const geometry_msgs::PoseStamped::ConstPtr &msg) {
  ...
  _vis->visualizePolyTraj(zero, traj_, 4.0);
  ROS_INFO("Visualizing optimization info");
  double max_vel = traj_.getMaxVelRate();
  double max_acc = traj_.getMaxAccRate();
